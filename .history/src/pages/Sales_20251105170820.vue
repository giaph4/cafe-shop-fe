...
const increaseItem = (it) => {
    ...
    updateOrderItem(activeOrder.value.id, it.id, { quantity: it.quantity + 1 }).then((data) => activeOrder.value = data).catch(() => toast.error('Không thể cập nhật'))
}
...
const removeItem = (it) => {
    if (!activeOrder.value) return
    deleteOrderItem(activeOrder.value.id, it.id).then((data) => activeOrder.value = data).catch(() => toast.error('Không thể xoá'))
}
...
const createOrSubmitOrder = async () => {
    try {
        ...
        const payload = {
            ...
            tableId: orderType.value === 'AT_TABLE' ? selectedTable.value?.id : null,
...
const applyVoucherToOrder = async () => {
    ...
    try { activeOrder.value = await applyVoucher(activeOrder.value.id, voucherCode.value) } catch (e) { toast.error('Voucher không hợp lệ') }
}
const removeVoucherFromOrder = async () => {
    if (!activeOrder.value) return
    try { activeOrder.value = await removeVoucher(activeOrder.value.id) } catch (e) { toast.error('Không thể gỡ voucher') }
}
