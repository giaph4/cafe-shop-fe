...
export const getProducts = async ({ page = 0, size = 12, sort = 'createdAt,desc', keyword = '', categoryId = null, status = null }) => {
    ...
    if (keyword) {
        // Gửi nhiều key phổ biến để tăng tương thích backend (server sẽ bỏ qua key lạ)
...
export const uploadProductImage = async (id, file) => {
    ...
    try {
        ...
        if (err.response?.status === 404) {
            // Fallback một số đường dẫn thường gặp
            const candidates = [
...
export const getProductRecipe = async (productId) => {
    try {
        ...
        if (err.response?.status === 404) {
            // Fallback: đôi khi backend đặt tên 'formula'
            const { data } = await api.get(`/products/${productId}/formula`)
            return data
        }
        throw err
    }
...
export const updateProductRecipe = async (productId, recipeItems) => {
    ...
    try {
        ...
        if (err.response?.status === 404) {
            const { data } = await api.put(`/products/${productId}/formula`, { items: recipeItems })
            return data
        }
        throw err
    }
