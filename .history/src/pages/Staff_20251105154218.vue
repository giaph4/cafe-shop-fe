                        ...
                        <div class="mb-3">
                            ...
                            <div v-else id="roleIdsGroup" class="p-3 bg-light rounded border"
                                ...
                                <div class="form-check" v-for="role in allRoles" :key="role.id">
                                    <Field name="roleIds" type="checkbox" :value="role.id" v-slot="{ field }">
                                        <input v-bind="field" class="form-check-input" :id="`role-${role.id}`"
...
const { data: rolesData, isLoading: allRolesLoading } = useQuery({
    ...
    queryFn: getAllRoles,
    onSuccess: (data) => {
        allRoles.value = Array.isArray(data) ? data : data?.data || []
    },
    staleTime: 1000 * 60 * 5
})
...
const toggleRole = (id) => {
  const index = formData.roleIds.indexOf(id)
  if (index === -1) formData.roleIds.push(id)
  else formData.roleIds.splice(index, 1)
}
...
const handleSubmit = (values) => {
  const payload = { ...values, roleIds: formData.roleIds }
