...
const selectedIngredientQty = ref(null)
...
// Tìm/Thêm nguyên liệu vào công thức
const loadIngredients = async () => {
    try {
        const data = await getIngredients(0, 50, ingredientSearch.value || '', 'name,asc')
        ingredientOptions.value = data.content || []
    } catch (e) {
        ingredientOptions.value = []
    }
}
const addIngredientToRecipe = () => {
    if (!selectedIngredientId.value || !selectedIngredientQty.value || selectedIngredientQty.value <= 0) {
        toast.warn('Chọn nguyên liệu và số lượng > 0')
        return
    }
    const ing = ingredientOptions.value.find(x => x.id === selectedIngredientId.value)
    if (!ing) return
    const existsIdx = recipeItems.value.findIndex(x => x.ingredientId === ing.id)
    if (existsIdx >= 0) {
        recipeItems.value[existsIdx].quantity = Number(recipeItems.value[existsIdx].quantity || 0) + Number(selectedIngredientQty.value)
    } else {
        recipeItems.value.push({ ingredientId: ing.id, ingredientName: ing.name, quantity: Number(selectedIngredientQty.value), unit: ing.unit })
    }
    selectedIngredientId.value = null
    selectedIngredientQty.value = null
}
