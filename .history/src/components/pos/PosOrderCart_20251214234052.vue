<template>
  ...
  <PosPaymentModal
    ...
    :show-payment-result="isPaymentResultModalOpen"
  ...
  <PaymentResultModal
    v-if="paymentResult"
    ref="paymentResultModalRef"
    :result="paymentResult"
    :order-info="paymentResultOrderInfo"
    :success-message="paymentSuccessMessage"
    :error-message="paymentErrorMessage"
    :show-view-detail="true"
    @continue="handlePaymentResultContinue"
    @view-detail="handlePaymentResultViewDetail"
    @retry="handlePaymentResultRetry"
    @close="handlePaymentResultClose"
  />
...
import PaymentResultModal from '@/components/common/PaymentResultModal.vue'
...
const paymentResultOrderInfo = ref(null)
const paymentSuccessMessage = ref('Cảm ơn bạn đã mua hàng. Đơn hàng của bạn đang được xử lý và sẽ giao sớm nhất.')
...
// Kiểm tra xem PaymentResultModal có đang mở không
const isPaymentResultModalOpen = computed(() => Boolean(paymentResult.value))
...
const confirmPayment = async ({ orderId, paymentMethod, tipAmount: tip, customerId, voucherCode } = {}) => {
    ...
    try {
        ...
        // Tạo timeout promise để xử lý timeout
        ...
        // Đóng tất cả modal khác trước khi mở PaymentResultModal
        await closeAllModals()

        // Sau đó mới set state và mở PaymentResultModal
        paymentResult.value = 'success'
        paymentResultOrderInfo.value = updatedOrder
        paymentSuccessMessage.value = 'Cảm ơn bạn đã mua hàng. Đơn hàng của bạn đang được xử lý và sẽ giao sớm nhất.'
        paymentErrorMessage.value = null

        // Hiển thị result modal
        await nextTick()
        paymentResultModalRef.value?.show()

        emitOrderUpdated('payment', updatedOrder)
    } catch (error) {
        // Xử lý timeout
        if (error?.message === 'TIMEOUT') {
            paymentResult.value = 'timeout'
            paymentErrorMessage.value = 'Yêu cầu thanh toán đã hết thời gian chờ. Vui lòng thử lại.'
        }
        // Xử lý network error
        else if (!error?.response || error?.code === 'ERR_NETWORK' || error?.message?.includes('Network') || error?.message?.includes('network')) {
            paymentResult.value = 'network_error'
            paymentErrorMessage.value = 'Không thể kết nối đến server. Vui lòng kiểm tra kết nối mạng và thử lại.'
        }
        ...
        // Sau đó mới set state và mở PaymentResultModal
        paymentResultOrderInfo.value = localOrder.value
        await nextTick()
        paymentResultModalRef.value?.show()
    } finally {
        loadingAction.value = null
...
const handlePaymentResultContinue = () => {
    ...
    paymentResultOrderInfo.value = null
    paymentErrorMessage.value = null
}

const handlePaymentResultViewDetail = async () => {
    // Lấy order ID từ payment result
    const orderId = paymentResultOrderInfo.value?.id
    if (!orderId) {
    ...
    // Đóng tất cả modal khác trước khi mở OrderDetailModal
    await closeAllModals()

    // Mở order detail modal
    await nextTick()
    if (orderDetailModalRef.value) {
...
const handlePaymentResultRetry = async () => {
    // Thử lại thanh toán
    paymentResult.value = null
    paymentResultOrderInfo.value = null
    paymentErrorMessage.value = null

    // Đóng tất cả modal khác trước khi mở lại payment modal
    await closeAllModals()

    // Mở lại payment modal
    if (localOrder.value?.id) {
        await nextTick()
        paymentModalRef.value?.show()
    }
...
.pos-cart__actions .btn-success {
	font-size: var(--font-size-lg); /* Tăng size */
	font-weight: var(--font-weight-bold); /* Bold */
	padding: var(--spacing-4) var(--spacing-5); /* Tăng padding */
	box-shadow: 0 4px 12px rgba(34, 197, 94, 0.3); /* Thêm shadow xanh lá */
	transition: all var(--transition-base);
}

.pos-cart__actions .btn-success:hover:not(:disabled) {
