...
const handleSelectConversation = async (conversation) => {
    ...
    if (wsConnection && wsConnection.isConnected && wsConnection.isConnected()) {
        ...
        // Nếu chưa kết nối, chờ kết nối rồi subscribe
        ...
        // Xóa interval sau 5 giây để tránh loop vô hạn
...
const loadMessages = async (beforeMessageId = null) => {
    ...
    if (beforeMessageId) {
        ...
        messagesPage.value = 0 // Reset trang khi tải tin nhắn ban đầu
    }
    try {
        ...
        if (beforeMessageId) {
            // Tải thêm tin nhắn cũ - thêm vào đầu danh sách
            messages.value = [...newMessages.reverse(), ...messages.value]
        } else {
            // Tải ban đầu - thay thế tất cả
...
const handleMessagesScroll = async (event) => {
    ...
    // Tải thêm khi cuộn lên đầu (trong phạm vi 100px)
    if (container.scrollTop < 100 && messages.value.length > 0) {
        ...
        if (firstMessage && !loadingMoreMessages.value) {
            ...
            // Khôi phục vị trí cuộn sau khi thêm tin nhắn mới
            await nextTick()
...
const handleConversationCreated = (conversation) => {
    // Thêm vào đầu và sắp xếp (ghim trước, sau đó theo updatedAt)
...
const handleNewMessage = (message) => {
    ...
    if (selectedConversation.value && message.conversationId === selectedConversation.value.id) {
        ...
        if (existingIndex === -1) {
            ...
            // Cập nhật tin nhắn hiện có (ví dụ: thay đổi trạng thái, thu hồi)
    ...
    if (index !== -1) {
        ...
        // Nếu cuộc hội thoại không có trong danh sách, tải lại
        loadConversations()
    }
}
...
const handleMessageSeen = (data) => {
    ...
    // Cập nhật trạng thái đã xem trong tin nhắn
