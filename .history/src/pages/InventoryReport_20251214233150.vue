  ...
  >
    ...
    <div class="card table-card">
      <div class="card-body p-0">
        <LoadingState v-if="tableData.loading.value" />
        <ErrorState
          v-else-if="tableData.error.value"
          :message="tableData.error.value?.message || 'Không thể tải dữ liệu báo cáo tồn kho'"
...
// Note: InventoryReport làm client-side pagination vì API trả về tất cả items
const tableData = useTableData({
    fetchFn: async () => {
        // API trả về tất cả items, không có pagination
        const response = await getInventoryReport(filters.lowStockOnly)
        return {
            content: response?.items || [],
            totalPages: 1,
            totalElements: response?.items?.length || 0
        }
    },
...
const inventoryItems = computed(() => tableData.data.value || [])

// Client-side search filtering
const baseItems = computed(() => {
    const searchKeyword = tableData.searchKeyword.value || ''
    if (!searchKeyword) return inventoryItems.value
...
// Cập nhật tableData với filtered items cho client-side pagination
watch(baseItems, (items) => {
    if (items.length !== tableData.data.value.length) {
        // Update data để trigger client-side pagination
        tableData.data.value = items
        tableData.totalElements.value = items.length
        tableData.totalPages.value = Math.ceil(items.length / pageSize.value) || 1
    }
}, { immediate: true })

// Table data sau khi pagination
const paginatedData = computed(() => {
    const start = zeroBasedPage.value * pageSize.value
    return baseItems.value.slice(start, start + pageSize.value)
})

// Lưu summary từ response
...
// Watch tableData để cập nhật summary
watch(() => tableData.data.value, async () => {
    // Fetch lại để lấy summary (hoặc tính từ data)
    try {
        const response = await getInventoryReport(filters.lowStockOnly)
        if (response?.summary) {
        ...
        // Ignore error, use computed summary
    }
}, { immediate: true })
...
// Error message được xử lý bởi ErrorState component
