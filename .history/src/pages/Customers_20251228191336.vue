    ...
    <div class="card tabs-card">
      <div class="card-body">
        ...
        <ErrorState
          ...
          :message="error && typeof error === 'object' ? error.message : error || 'Có lỗi xảy ra'"
...
// Tự quản lý pagination state (không dùng useTableData, giống Products.vue)
...
const allCustomers = ref([]) // Tất cả customers từ API
...
// Fetch tất cả customers (với size lớn để có đủ dữ liệu filter)
    ...
    await execute(async () => {
        ...
        if (response.totalPages > 1) {
            ...
            for (let page = 1; page < response.totalPages && page < 10; page++) {
                ...
                const pageResponse = await getCustomers(pageParams)
                if (pageResponse.content) {
                    allPages.push(...pageResponse.content)
                }
            }

            allCustomers.value = allPages
        } else {
...
// Áp dụng filter và sort cho customers (client-side)
const applyFilters = (customerList) => {
    ...
    if (searchKeyword.value?.trim()) {
        ...
        result = result.filter(c => {
            ...
            return fullName.includes(keyword) || phone.includes(keyword) || email.includes(keyword)
        })
    }

    // Filter theo loyalty points
    if (filters.minLoyaltyPoints !== null && filters.minLoyaltyPoints !== undefined && filters.minLoyaltyPoints > 0) {
        result = result.filter(c => (Number(c.loyaltyPoints) || 0) >= filters.minLoyaltyPoints)
    }
    if (filters.maxLoyaltyPoints !== null && filters.maxLoyaltyPoints !== undefined && filters.maxLoyaltyPoints > 0) {
        result = result.filter(c => (Number(c.loyaltyPoints) || 0) <= filters.maxLoyaltyPoints)
    }

    // Filter theo created date
...
// Computed để tự động cập nhật từ allCustomers và apply client-side filters + pagination
const customers = computed(() => {
    const rawCustomers = allCustomers.value || []
    const filtered = applyFilters(rawCustomers)
...
const openDetailDrawer = (customerId) => {
...
const handlePageChange = (page) => {
    ...
    // customers sẽ tự động cập nhật qua computed khi zeroBasedPage thay đổi
}
...
// Watch cho search keyword
watch(searchKeyword, () => {
    syncToUrl()
})

// Watch cho các filter (reset về trang đầu, customers sẽ tự động cập nhật qua computed)
watch(
    () => [filters.minLoyaltyPoints, filters.maxLoyaltyPoints, filters.createdDateFrom, filters.createdDateTo],
    () => {
        setPageFromZero(0)
    }
)

// Watch cho activeTab để fetch data khi cần
watch(
    () => activeTab.value,
    (newTab) => {
        if (newTab === 'list') {
