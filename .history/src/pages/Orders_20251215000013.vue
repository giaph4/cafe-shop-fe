<template>
  ...
  >
    ...
    >
      <div class="card-body">
        <!-- Basic Filters - Tối ưu bố cục để giảm khoảng trắng -->
        <div class="row g-3 align-items-end">
          ...
          <div class="col-lg-3 col-md-12 col-sm-12 d-flex align-items-end justify-content-end gap-2">
            <button
              class="btn btn-link text-muted text-decoration-none p-0 small"
              type="button"
              :disabled="loading"
...
// Sử dụng computed để tự động cập nhật từ tableData.data và apply client-side filters
const orders = computed(() => {
    if (activeTab.value !== 'list') return []
    const rawOrders = tableData.data.value || []
    return applyClientSideFilters(rawOrders)
})
...
const fetchOrders = async () => {
    ...
    // orders sẽ tự động cập nhật qua computed
...
const handlePrintOrder = (order) => {
    if (!order) return
    // Mở modal chi tiết đơn để in
    selectedOrderId.value = order.id
    orderDetailModal.value?.show()
    // Sau khi modal mở, có thể trigger print từ modal
    // Hoặc có thể tạo một component print riêng
    toast.info('Tính năng in đơn sẽ được mở trong modal chi tiết')
}

// Handle pay order
const handlePayOrder = (order) => {
    if (!order || order.status !== 'PENDING') {
        toast.warning('Chỉ có thể thanh toán đơn hàng đang chờ')
        return
    }
    // Mở modal thanh toán (có thể dùng PosPaymentModal hoặc tạo modal riêng)
    selectedOrderId.value = order.id
    selectedOrder.value = order
    // TODO: Mở modal thanh toán
    toast.info('Tính năng thanh toán đơn hàng đang được phát triển')
}
...
// orders đã là computed, tự động cập nhật khi tableData.data thay đổi
...
.filter-card .btn-link {
    color: var(--color-text-primaryprimary);
    font-size: var(--font-size-base);
    font-weight: var(--font-weight-medium);
    font-family: var(--font-family-sans);
    transition: all var(--transition-base);
