<template>
            ...
            <div class="modal-body">
              ...
              <div class="row">
                <div class="col-md-6 mb-3">
                  <label class="form-label fw-bold">Số tiền (VND) <span class="text-danger">*</span></label>
                  <Field 
                    name="amount" 
                    type="number" 
                    class="form-control"
  ...
  // API 53: Lấy chi phí (sẽ tự gọi lại khi 'filters' hoặc 'currentPage' thay đổi)
  ...
  // Tự động reset về trang 1 khi filter
  watch(filters, () => {
    currentPage.value = 1
  })
  ...
  const openModal = (expense = null) => {
    if (expense) {
      ...
      // API trả về mảng [YYYY, MM, DD], cần chuyển đổi
      formData.expenseDate = new Date(expense.expenseDate).toISOString().split('T')[0]
    } else {
      isEditing.value = false
      Object.assign(formData, initialFormData)
      formData.expenseDate = new Date().toISOString().split('T')[0] // Đặt lại ngày hôm nay
    }
    bsModal.value?.show()
  }
