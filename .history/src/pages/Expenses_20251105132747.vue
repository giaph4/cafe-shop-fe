<template>
    <div class="card">
        ...
        <Toolbar class="mb-4">
            ...
            <template #end>
                <div class="flex flex-wrap gap-2 align-items-center">
                    <Calendar v-model="dateFilters.startDate" placeholder="Từ ngày" dateFormat="dd/mm/yy" showIcon class="mr-2" />
                    <Calendar v-model="dateFilters.endDate" placeholder="Đến ngày" dateFormat="dd/mm/yy" showIcon class="mr-2" />
                    <Button label="Lọc" icon="pi pi-filter" @click="onFilter" />
                    <Button label="Xóa lọc" icon="pi pi-filter-slash" outlined severity="secondary" @click="clearFilters" class="ml-2" />
                </div>
            </template>
        </Toolbar>
        ...
        <DataTable
            ...
            filterDisplay="row"
            :globalFilterFields="['category', 'description', 'username']"
            @page="onPage($event)"
            @sort="onSort($event)"
        >
            <template #header>
                <div class="flex justify-content-between align-items-center">
                    ...
                    <span class="p-input-icon-left">
                        ...
                        <InputText v-model="filters['global'].value" placeholder="Tìm kiếm theo hạng mục, mô tả, người tạo..." />
                    </span>
                </div>
            </template>
            ...
            <Column field="category" header="Hạng mục" sortable filterField="category" style="width: 15%">
                 <template #filter="{ filterModel, filterCallback }">
                    <InputText v-model="filterModel.value" type="text" @input="filterCallback()" class="p-column-filter" placeholder="Lọc theo hạng mục" />
                </template>
            </Column>
            ...
            <Column field="username" header="Người tạo" sortable filterField="username" style="width: 15%">
                <template #filter="{ filterModel, filterCallback }">
                    <InputText v-model="filterModel.value" type="text" @input="filterCallback()" class="p-column-filter" placeholder="Lọc theo người tạo" />
                </template>
            </Column>
            ...
            <Column :exportable="false" style="width: 10%">
                <template #body="slotProps">
                    <Button icon="pi pi-pencil" outlined rounded severity="info" class="mr-2" @click="editExpense(slotProps.data)" />
        ...
        <Dialog v-model:visible="expenseDialog" :style="{ width: '450px' }" :header="dialogHeader" :modal="true" class="p-fluid">
            
            <div class="field">
                <label for="category">Hạng mục chi phí</label>
                <InputText id="category" v-model.trim="expense.category" required="true" autofocus :class="{ 'p-invalid': submitted && !expense.category }" />
                <small class="p-error" v-if="submitted && !expense.category">Hạng mục là bắt buộc.</small>
            </div>

            <div class="field">
                <label for="amount">Số tiền</label>
                <InputNumber id="amount" v-model="expense.amount" mode="currency" currency="VND" locale="vi-VN" required="true" :class="{ 'p-invalid': submitted && (!expense.amount || expense.amount <= 0) }" />
                <small class="p-error" v-if="submitted && (!expense.amount || expense.amount <= 0)">Số tiền là bắt buộc và phải lớn hơn 0.</small>
            </div>

            <div class="field">
                <label for="expenseDate">Ngày chi</label>
                <Calendar id="expenseDate" v-model="expense.expenseDate" dateFormat="dd/mm/yy" showIcon required="true" :class="{ 'p-invalid': submitted && !expense.expenseDate }" />
        ...
        <Dialog v-model:visible="deleteExpenseDialog" :style="{ width: '450px' }" header="Xác nhận" :modal="true">
            <div class="confirmation-content">
                ...
                <span v-if="expense">Bạn có chắc chắn muốn xóa chi phí <b>{{ expense.category }}</b> ({{ formatCurrency(expense.amount) }}) không?</span>
            </div>
            <template #footer>
...
import { expenseService } from '../api/expenseService'; // <-- FILE NÀY PHẢI ĐƯỢC TẠO (xem mục 2)
import { formatMoney } from '../utils/formatMoney'; // <-- FILE NÀY ĐÃ TỒN TẠI (OK)
