...
const statusMutation = useMutation({
    ...
    onSuccess: (data) => {
        ...
        // Cập nhật cache của Vue Query ngay lập tức thay vì gọi lại API
        queryClient.setQueryData(['tables'], (oldData) => {
            return oldData.map(table => table.id === data.id ? data : table)
        })
    },
    onError: (err, variables) => {
        toast.error(err.response?.data?.message || 'Lỗi khi cập nhật trạng thái.')
        // Nếu lỗi, chúng ta tải lại toàn bộ list để đảm bảo dropdown quay về giá trị đúng
        queryClient.invalidateQueries(['tables'])
    }
})
