            ...
            <tbody>
              ...
              >
                ...
                <td>
                  <div
                    class="fw-semibold"
                    :class="adjustment.type === 'BONUS' ? 'text-success' : 'text-danger'"
                  >
                ...
                <td>
                  <span
                    class="badge"
                    :class="adjustment.revoked ? 'bg-secondary' : 'bg-success'"
                  >
                    {{ adjustment.revoked ? 'Đã thu hồi' : 'Đang hiệu lực' }}
                  </span>
                  <div
                    v-if="adjustment.revoked && adjustment.revokedAt"
                ...
                <td class="text-end">
                  <div class="btn-group">
                    ...
                    >
                      <i class="bi bi-x-circle" /> Thu hồi
                    </button>
                    <button
                      class="btn btn-sm btn-outline-danger"
                      @click="handleRemove(adjustment)"
                    >
                      <i class="bi bi-trash" />
                    </button>
                  </div>
...
const assignmentId = computed({
    get: () => localFilters.value.assignmentId,
    set: (value) => {
        emit('update:filters', { ...localFilters.value, assignmentId: value })
        emit('filter')
    }
})

const type = computed({
    get: () => localFilters.value.type,
    set: (value) => {
        emit('update:filters', { ...localFilters.value, type: value })
        emit('filter')
    }
})

const revoked = computed({
    get: () => localFilters.value.revoked,
    set: (value) => {
        emit('update:filters', { ...localFilters.value, revoked: value })
        emit('filter')
    }
})
