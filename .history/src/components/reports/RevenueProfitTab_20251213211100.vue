...
// Build hourly data - chỉ hiển thị giờ có giao dịch (ẩn giờ chết 0h-6h, 23h)
const fullHourlyData = computed(() => {
    const sales = Array.isArray(props.hourlySales) ? props.hourlySales : []
    const hoursMap = new Map()

    // Chỉ lấy giờ có giao dịch hoặc trong khoảng 7h-22h (giờ hoạt động của quán)
    sales.forEach(item => {
        const hour = Number(item?.hour)
        if (!isNaN(hour) && hour >= 0 && hour < 24) {
            const revenue = Number(item?.revenue) || Number(item?.totalRevenue) || 0
            const orderCount = Number(item?.orderCount) || 0
            // Chỉ thêm giờ có giao dịch hoặc trong giờ hoạt động (7h-22h)
            if (revenue > 0 || orderCount > 0 || (hour >= 7 && hour <= 22)) {
                hoursMap.set(hour, {
                    hour,
                    revenue,
                    orderCount
        ...
        xaxis: {
            ...
            labels: {
                ...
                formatter: (value, opts) => {
                    // Nếu value là index, lấy category tương ứng
                    const index = opts?.dataPointIndex ?? opts?.seriesIndex ?? 0
                    if (formattedCategories[index]) {
                        return formattedCategories[index]
                    }
                    // Nếu value là string date, format nó
...
const hourlyChartOptions = computed(() => {
    ...
    return {
        ...
        colors: [REVENUE_COLOR, '#64748b'], // Doanh thu = xanh dương, Số đơn = xám
        xaxis: {
            categories: hours,
            labels: {
                style: {
        ...
        tooltip: {
            ...
            y: {
                formatter: (val, { seriesIndex }) => {
                    if (seriesIndex === 0) {
                        return formatCurrency(val) // Tooltip vẫn hiện đầy đủ
                    }
                    return `${val} đơn`
                }
            },
