<template>
    <div class="voucher-analytics-page page-container container-fluid" data-aos="fade-up">
        <PageHeader
            ...
            <div class="card-body">
                <div class="row g-3 align-items-end">
                    ...
                    <div class="col-lg-3 col-md-6">
                        ...
                        <select class="form-select clean-input" v-model="filters.voucherId">
        ...
        <LoadingState v-if="loading" text="Đang phân tích dữ liệu voucher..." />
        <ErrorState
            v-else-if="error"
            :message="error"
            @retry="handleAnalyze"
        ...
        <div v-else-if="analyticsData" class="analytics-content">
                ...
                <div class="col-lg-3 col-md-6">
                    <div class="kpi-card kpi-card--primary">
                        ...
                        <div class="kpi-card__content">
                            <div class="kpi-card__label">Tổng voucher</div>
                            <div class="kpi-card__value">{{ summary?.totalVouchers || 0 }}</div>
                            <div class="kpi-card__subtitle">{{ summary?.activeVouchers || 0 }} đang hoạt động</div>
                        </div>
                    </div>
                </div>
                <div class="col-lg-3 col-md-6">
                    <div class="kpi-card kpi-card--success">
                        ...
                        <div class="kpi-card__content">
                            <div class="kpi-card__label">Tổng sử dụng</div>
                            <div class="kpi-card__value">{{ formatNumber(summary?.totalUsage || 0) }}</div>
                            <div class="kpi-card__subtitle">Lần sử dụng</div>
                        </div>
                    </div>
                </div>
                <div class="col-lg-3 col-md-6">
                    <div class="kpi-card kpi-card--warning">
                        ...
                        <div class="kpi-card__content">
                            <div class="kpi-card__label">Tổng giảm giá</div>
                            <div class="kpi-card__value">{{ formatCurrency(summary?.totalDiscount || 0) }}</div>
                            <div class="kpi-card__subtitle">Đã chiết khấu</div>
                        </div>
                    </div>
                </div>
                <div class="col-lg-3 col-md-6">
                    <div class="kpi-card" :class="(summary?.avgROI || 0) >= 0 ? 'kpi-card--success' : 'kpi-card--danger'">
                        <div class="kpi-card__icon">
                            <i class="bi bi-graph-up-arrow"></i>
                        </div>
                        <div class="kpi-card__content">
                            <div class="kpi-card__label">ROI trung bình</div>
                            <div class="kpi-card__value">{{ (summary?.avgROI || 0).toFixed(2) }}%</div>
                            <div class="kpi-card__subtitle">Hiệu quả đầu tư</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card standard-card mb-4">
                <div class="card-header standard-card-header">
                    <h5 class="card-title">Biểu đồ sử dụng voucher</h5>
                    <button
            ...
            <div class="card standard-card">
                <div class="card-header standard-card-header">
                    <h5 class="card-title">Bảng so sánh voucher</h5>
                </div>
                <div class="card-body">
                        ...
                        <table class="table table-minimal">
                            ...
                            <tbody>
                                <tr v-for="voucher in vouchers" :key="voucher.voucherId">
                                    ...
                                    <td>
                                        <span class="badge badge-soft" :class="voucher.type === 'FIXED_AMOUNT' ? 'badge-primary' : 'badge-info'">
                                            {{ voucher.type === 'FIXED_AMOUNT' ? 'Cố định' : 'Phần trăm' }}
                                        </span>
                                    </td>
                                    <td class="fw-semibold usage-count">{{ formatNumber(voucher.metrics.totalUsage) }}</td>
                                    <td class="discount-amount">{{ formatCurrency(voucher.metrics.totalDiscount) }}</td>
                                    <td class="revenue-amount">{{ formatCurrency(voucher.metrics.totalRevenue) }}</td>
                                    <td>
                                        <span class="badge badge-soft" :class="voucher.metrics.roi >= 0 ? 'badge-success' : 'badge-danger'">
                                            {{ voucher.metrics.roi.toFixed(2) }}%
                                        </span>
                                    </td>
                                    <td class="customers-count">{{ formatNumber(voucher.metrics.uniqueCustomers) }}</td>
                                    <td>
                                        <span class="badge badge-soft" :class="voucher.active ? 'badge-success' : 'badge-neutral'">
