<template>
    ...
    <div class="modal fade" id="staffModal" tabindex="-1" ref="modalElement" aria-labelledby="staffModalLabel" aria-hidden="true">
          ...
          <Form @submit="handleSubmit" :validation-schema="staffSchema" v-slot="{ errors }">
            <div class="modal-body">
              ...
              <div class="mb-3">
                ...
                <div v-else id="roleIdsGroup" class="p-3 bg-light rounded border" :class="{ 'is-invalid': errors.roleIds }">
      ...
      <div class="card mb-4">
        <div class="card-body">
          <div class="input-group" style="max-width: 400px;">
            ...
            <input type="text" class="form-control" placeholder="Tìm theo tên, SĐT, email..." v-model="searchQuery" />
          </div>
        </div>
      </div>
          ...
          <div v-else-if="pageData && pageData.content" class="table-responsive">
            <table class="table table-hover align-middle">
              <thead class="table-light">
                <tr>
                  <th @click="handleSort('id')" class="sortable">ID <i :class="getSortIcon('id')"></i></th>
                  <th @click="handleSort('username')" class="sortable">Tên đăng nhập <i :class="getSortIcon('username')"></i></th>
                  <th @click="handleSort('fullName')" class="sortable">Họ tên <i :class="getSortIcon('fullName')"></i></th>
                  <th>Liên hệ</th>
                  <th>Quyền</th>
                  <th @click="handleSort('status')" class="sortable">Trạng thái <i :class="getSortIcon('status')"></i></th>
              ...
              <tbody>
                <tr v-for="user in filteredUsers" :key="user.id">
                  ...
                  <td>
                    <span v-for="role in user.roles" :key="role.id" class="badge me-1" :class="getRoleClass(role.name)">
                      {{ role.name }}
                    </span>
                  </td>
                  <td>
  ...
  .sortable i { margin-left: 4px; font-size: 0.8em; color: #aaa; }
  .sortable:hover i { color: #A36B4A; }
  #roleIdsGroup.is-invalid { border-color: var(--bs-danger) !important; }
  </style>
