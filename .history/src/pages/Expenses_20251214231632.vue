      ...
      <div class="card table-card">
        <div class="card-body">
          ...
          <ErrorState
            ...
            :message="tableData.error.value?.response?.data?.message || tableData.error.value?.message || 'Không thể tải dữ liệu chi phí'"
            @retry="tableData.fetchData"
          />
          <template v-else-if="expenses.length > 0 || filteredExpenses.length > 0">
            <div
        ...
        <div
          v-if="filteredExpenses.length > 0 && totalPages > 1"
...
<script setup>
import { computed, onMounted, onUnmounted, reactive, ref, watch, Teleport } from 'vue'
...
const currentPage = computed(() => tableData.currentPage.value)
...
// Lấy dữ liệu từ useTableData và filter client-side
const expenses = computed(() => tableData.data.value || [])
