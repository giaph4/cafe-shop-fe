...
const openModal = async (user) => {
    await loadAllRoles() // đảm bảo roles đã được load
    formData.id = user.id
    ...
    setValues({ ...formData }) // cập nhật giá trị form
    bsModal.value?.show()
}
...
// Toggle role id vào/ra mảng roleIds và đồng bộ lại form
const toggleRole = (roleId) => {
    ...
    // Cập nhật lại giá trị cho vee-validate để field roleIds nhận thay đổi
    setValues({ ...formData })
}
