...
const PAYMENT_METHOD_LABELS = Object.freeze({
    ...
    TRANSFER: 'Chuyển khoản', // Map TRANSFER (từ API) thành Chuyển khoản
    E_WALLET: 'Ví điện tử',
    QRIS: 'Thanh toán QR',
    OTHER: 'Khác'
})
...
export const getPaymentMethodStats = async (startDate, endDate) => {
    ...
    const enriched = items.map((item) => {
        // Normalize payment method để xử lý cả TRANSFER và BANK_TRANSFER
        const normalizedMethod = String(item.paymentMethod || '').toUpperCase()
        const methodKey = normalizedMethod === 'TRANSFER' ? 'TRANSFER' : normalizedMethod
        const label = PAYMENT_METHOD_LABELS[methodKey] ?? PAYMENT_METHOD_LABELS[normalizedMethod] ?? item.paymentMethod ?? PAYMENT_METHOD_LABELS.OTHER
