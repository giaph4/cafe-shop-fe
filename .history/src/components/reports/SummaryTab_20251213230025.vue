    ...
    return [
        {
            ...
            subtitle: comparison ? `${todayGrowth >= 0 ? '+' : ''}${todayGrowth.toFixed(2)}% so với kỳ trước` : null,
        ...
        subtitle: card.subtitle || (card.key === 'todayRevenue' && props.dailyRevenue && !card.subtitle
...
const productChartSeries = computed(() => {
    if (!topCategoryItems.value.length) {
    ...
    const values = topCategoryItems.value.map((item) => {
...
const productChartOptions = computed(() => {
    ...
    return mergeOptions(base, {
        ...
        // Với horizontal bar: categories ở yaxis, giá trị ở xaxis
        yaxis: resolvedProductChartType.value === 'bar'
            ? {
                ...base.yaxis,
                categories: isHorizontal && labels.length > 0 ? labels : undefined,
                labels: {
                    ...base.yaxis.labels,
                    style: {
...
const customerChartOptions = computed(() => {
    ...
    return mergeOptions(base, {
        ...
        // Với horizontal bar: categories ở yaxis, giá trị ở xaxis
        yaxis: resolvedCustomerChartType.value === 'bar'
            ? {
                ...base.yaxis,
                categories: isHorizontal && labels.length > 0 ? labels : undefined,
                labels: {
...
.reports-summary {
    ...
    gap: 1.5rem; /* Tăng từ 1.25rem lên 1.5rem */
}

/* Tăng khoảng cách đặc biệt giữa Điểm nổi bật và biểu đồ */
.summary-insights {
    margin-bottom: 0.5rem; /* Thêm margin-bottom để tách biệt rõ ràng hơn */
}
