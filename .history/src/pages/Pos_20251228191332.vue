          ...
          <section class="pos-order-section pos-order-section--collapsible">
            ...
            >
              <div>
                ...
                >
                  Đơn đã lưu, đang chờ thanh toán. Chọn để tiếp tục xử lý.
                </p>
              </div>
              <div class="pos-order-section__header-actions">
                <button
                  ...
                  title="Làm mới danh sách đơn chờ xử lý"
                >
                  <span
          ...
          <section class="pos-order-section pos-order-section--collapsible">
            ...
            >
              <div>
                ...
                <p
                  v-if="!isDraftSectionCollapsed"
                  class="pos-order-section__subtitle"
                >
                  Đơn đang soạn, chưa lưu. Bấm "Lưu đơn hàng" để chuyển sang đơn chờ.
                </p>
              </div>
              <i
                class="bi pos-order-section__collapse-icon"
...
// Sử dụng useCollapseState để lưu trạng thái collapse vào localStorage
// Trạng thái sẽ được tự động lưu và khôi phục khi F5
const isRightPanelCollapsed = useCollapseState('pos_right_panel_collapsed', false)
const isTakeawaySectionCollapsed = useCollapseState('pos_takeaway_section_collapsed', true)
const isDraftSectionCollapsed = useCollapseState('pos_draft_section_collapsed', true)
...
const handleRefreshTakeawayOrders = async () => {
    try {
        ...
        const result = await refetchTakeawayOrders()
        
        // Thông báo thành công
        if (result?.data) {
            const count = pendingTakeawayOrders.value.length
            if (count > 0) {
                toast.success(`Đã làm mới. Có ${count} đơn đang chờ xử lý.`)
            } else {
                toast.info('Đã làm mới. Không có đơn nào đang chờ.')
            }
        }
    } catch (error) {
        logger.error('Failed to refresh takeaway orders:', error)
        toast.error('Không thể làm mới danh sách đơn. Vui lòng thử lại.')
    }
}
...
const initializeFromRoute = async () => {
    const { tableId, orderId, action } = route.query
...
// Cleanup modal và backdrop khi mount
const cleanupModals = () => {
...
onMounted(async () => {
    // Cleanup modal và backdrop trước khi load
    cleanupModals()
