        ...
        <div v-else>
          ...
          >
            <template #header>
              ...
              <th class="col-actions text-center action-column-header">
                Hành động
              </th>
            </template>
                ...
                <div class="status-switch-wrapper">
                  ...
                  >
                    <input
                      ...
                      @change="handleToggleAvailability(product)"
                    >
                    <label
                      class="form-check-label status-switch-label"
                      :for="`status-switch-${product.id}`"
                    >
                      <span
                        class="badge badge-status badge-status--inline"
                        :class="product.available ? 'badge-status--success' : 'badge-status--danger'"
                      >
            ...
            <table class="table align-middle table-hover table-sticky-header">
              <thead>
                <tr>
                  ...
                  <th class="text-center action-column-header col-actions">
                    Hành động
                  </th>
                </tr>
              </thead>
            ...
            <template #default="{ item: product }">
              <article class="product-card">
                ...
                >
                  <div class="dropdown">
                    <button
                      class="btn btn-sm btn-outline-secondary product-card__menu-btn"
                      type="button"
                      data-bs-toggle="dropdown"
                      aria-expanded="false"
                      title="Thao tác"
...
const handleToggleAvailability = async (product) => {
    if (!product?.id) return
    const originalValue = product.available
    ...
    try {
        ...
        // Revert lại giá trị ban đầu nếu có lỗi
        product.available = originalValue
    } finally {
...
.status-switch-wrapper .form-check {
    display: flex;
    align-items: center;
    gap: var(--spacing-2);
    margin-bottom: 0;
...
.status-switch-wrapper .form-check-input {
    width: 48px;
    height: 24px;
    cursor: pointer;
    margin: 0;
...
.status-switch-wrapper .form-check-label {
    cursor: pointer;
    margin: 0;
    padding: 0;
}
...
/* Action Column - Tối ưu khoảng cách */
.action-column {
    width: 120px;
    min-width: 120px;
    max-width: 120px;
