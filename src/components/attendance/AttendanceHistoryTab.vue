      ...
      <div class="card-header border-0 d-flex flex-wrap justify-content-between align-items-center">
        <div>
          ...
          <p class="text-muted mb-0">
            Danh sách chi tiết các lần chấm công
          </p>
        </div>
      </div>
      <div class="card-body">
          ...
          <table class="table table-hover">
            ...
            <tbody>
              ...
              >
                <td>
                  <div class="d-flex flex-column">
                    ...
                    <small
                      ...
                      class="text-muted"
                    >Bởi: {{ record.createdBy }}</small>
                  </div>
                </td>
                <td>
                  <div
                    v-if="record.checkOutAt"
                    class="d-flex flex-column"
                  >
                    <strong>{{ formatDateTime(record.checkOutAt) }}</strong>
                    <small
                      v-if="record.updatedBy"
                      class="text-muted"
                    >Bởi: {{ record.updatedBy }}</small>
                  </div>
                  <span
                    v-else
                    class="text-muted"
                ...
                <td>
                  <span
                    v-if="record.lateMinutes && record.lateMinutes > 0"
                    class="text-warning"
                  >
                    <i class="bi bi-clock me-1" />{{ record.lateMinutes }} phút
                  </span>
                  <span
                    v-else
                    class="text-success"
                  >
                    <i class="bi bi-check-circle me-1" />Đúng giờ
                  </span>
                </td>
                <td>
                  <span
                    v-if="record.earlyLeaveMinutes && record.earlyLeaveMinutes > 0"
                    class="text-warning"
                  >
                    <i class="bi bi-clock me-1" />{{ record.earlyLeaveMinutes }} phút
                  </span>
                  <span
                    v-else-if="record.checkOutAt"
                    class="text-success"
                  >
                    <i class="bi bi-check-circle me-1" />Đúng giờ
                  </span>
...
const checkedInCount = computed(
    () => props.records?.filter(r => r.checkInAt).length || 0
)
const checkedOutCount = computed(
    () => props.records?.filter(r => r.checkOutAt).length || 0
)
const lateCount = computed(
    () =>
        props.records?.filter(r => r.lateMinutes && r.lateMinutes > 0).length || 0
)
