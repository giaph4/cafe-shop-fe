                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <Field name="unit" type="text" class="form-control" placeholder="ví dụ: kg, lít, cái"
    ...
    <div class="modal fade" id="adjustModal" tabindex="-1" ref="adjustModalElement">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <Form @submit="handleAdjustSubmit" :validation-schema="adjustSchema" v-slot="{ errors, setValues }">
                    <div class="modal-body">
                        <h6 class="mb-3">Nguyên liệu: <span class="text-primary">{{ adjustData.name }}</span></h6>
                        <div class="mb-3">
                            <label class="form-label fw-bold">Lượng điều chỉnh <span
                                    class="text-danger">*</span></label>
                            <Field name="adjustment" type="number" step="0.01" class="form-control"
                                placeholder="Nhập số âm để giảm, số dương để tăng"
                                :class="{ 'is-invalid': errors.adjustment }" v-model="adjustData.adjustment" />
                            <ErrorMessage name="adjustment" class="invalid-feedback" />
                        </div>
                        <div class="mb-3">
                            ...
                            <Field name="reason" as="textarea" rows="3" class="form-control"
                                placeholder="ví dụ: Hỏng, Kiểm kho,..." :class="{ 'is-invalid': errors.reason }"
        ...
        <div class="card mb-4">
            <div class="card-body">
                <div class="input-group" style="max-width: 400px;">
                    ...
                    <input type="text" class="form-control" placeholder="Tìm kiếm theo tên nguyên liệu..."
                        v-model="searchQuery">
                </div>
            </div>
        </div>
...
const adjustData = reactive({ ingredientId: null, name: '', adjustment: null, reason: '' })
...
const adjustSchema = yup.object({
    adjustment: yup.number().required('Lượng điều chỉnh là bắt buộc').notOneOf([0], 'Không thể là 0'),
...
// === Vue Query: Mutations (API 35, 38, 39, 40) ===
...
const openAdjustModal = (ingredient) => {
    ...
    adjustData.adjustment = null // Reset form
    adjustData.reason = ''       // Reset form
...
// ===================================
// Bị lỗi vì hàm `handleAdjustSubmit` không nhận `values`
// Nó đã lấy `adjustment` từ `v-model` (là string)
// Sửa lại: Phải nhận `values` (đã được yup cast sang number)
const handleAdjustSubmit = (values) => {
    adjustMutation.mutate({
        ingredientId: adjustData.ingredientId, // ID lấy từ state
        adjustment: values.adjustment,       // Lấy adjustment ĐÃ CAST (number) từ values
        reason: values.reason              // Lấy reason ĐÃ VALIDATE từ values
    })
}
