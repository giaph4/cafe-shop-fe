...
const fetchOrders = async () => {
    ...
    updateOrdersFromTableData()
}

// Cập nhật orders từ tableData.data và apply client-side filters
const updateOrdersFromTableData = () => {
    const rawOrders = tableData.data.value || []
...
// Watch tableData.data để tự động cập nhật orders khi page thay đổi
watch(
    () => [tableData.data.value, activeTab.value],
    () => {
        if (activeTab.value === 'list' && tableData.data.value) {
            updateOrdersFromTableData()
        }
    },
