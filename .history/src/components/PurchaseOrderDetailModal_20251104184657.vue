            ...
            <div v-if="data">
              <div class="row mb-3">
                ...
                <div class="col-md-6">
                  ...
                  <p>{{ data.staffUsername }}</p>
                </div>
                <div class="col-md-4">
                  <strong>Ngày đặt:</strong>
                  <p>{{ new Date(data.orderDate).toLocaleString('vi-VN') }}</p>
                </div>
                <div class="col-md-4">
              ...
              <div class="table-responsive">
                <table class="table table-sm table-bordered">
                  ...
                  <tbody>
                    <tr v-for="item in data.purchaseOrderDetails" :key="item.id">
                      <td>{{ item.ingredientName }}</td>
                      <td class="text-end">{{ item.quantity }} ({{ item.ingredientUnit }})</td>
                      <td class="text-end">{{ formatMoney(item.unitPrice) }}</td>
                      <td class="text-end fw-bold">{{ formatMoney(item.quantity * item.unitPrice) }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
