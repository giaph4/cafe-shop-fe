                    ...
                    <table class="table table-hover align-middle">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th scope="col" @click="handleSort('expenseDate')" class="sortable">
                                    Ngày chi <i :class="getSortIcon('expenseDate')"></i>
                                </th>
                                <th scope="col" @click="handleSort('category')" class="sortable">
                                    Hạng mục <i :class="getSortIcon('category')"></i>
                                </th>
                                <th scope="col" @click="handleSort('amount')" class="sortable">
                                    Số tiền <i :class="getSortIcon('amount')"></i>
                                </th>
                                <th scope="col" @click="handleSort('user.username')" class="sortable">
...
// --- State theo phong cách Ingredients.vue ---
...
// Thêm state cho Lọc ngày (Server-side)
const startDate = ref('')
const endDate = ref('')
...
// --- Gán giá trị mặc định cho bộ lọc ngày ---
...
const { data: pageData, isLoading, isError, error } = useQuery({
    // queryKey theo dõi các state server-side
    queryKey: ['expenses', currentPage, sortState, startDate, endDate],
    queryFn: () => {
        // Xây dựng chuỗi sort
        const sortString = `${sortState.key},${sortState.direction}`

        // Gọi API với object filters
        return getExpenses({
            page: currentPage.value - 1, // API 0-indexed
            size: 10, // Giữ cố định 10
            sort: sortString,
...
// Lọc Client-side (Tìm kiếm) - vì API không hỗ trợ
...
const handleSort = (key) => {
    if (sortState.key === key) {
        ...
        sortState.key = key
        sortState.direction = 'desc' // Mặc định desc
    }
}
...
// Reset về trang 1 khi lọc hoặc sort (dùng deep watch cho sortState)
watch([sortState, startDate, endDate], () => {
    currentPage.value = 1
}, { deep: true })
