...
export const getRealTimeDashboard = async (userRole, userId = null) => {
    try {
        ...
        const [
            ...
            inventoryService.analyzeInventory({ includeStable: false }).then(data => {
        ...
        const hourlyRevenue = generateHourlyRevenue(todayStr)
...
const getRoleDashboard = async (userRole, userId) => {
    try {
        ...
        console.warn('[RealTimeDashboard] Failed to get role dashboard, using defaults', error)
...
const generateHourlyRevenue = (date) => {
    const hours = []
    const now = new Date()
