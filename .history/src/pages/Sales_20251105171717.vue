            ...
            <div class="col-12 col-lg-8">
                <div class="card">
                        ...
                        <div v-else class="row g-3">
                            <div class="col-6 col-md-4 col-lg-3" v-for="p in productsPage?.content || []" :key="p.id">
                                <div class="card h-100 product-tile" @click="addToCart(p)">
                                    <div class="ratio ratio-4x3"><img :src="getImageUrl(p)" class="card-img-top" />
                                    </div>
                                    <div class="card-body">
                    ...
                    <div class="card-footer d-flex justify-content-end">
                        <Pagination v-if="productsPage" :current-page="productPage"
                            :total-pages="productsPage.totalPages" @page-change="onChangeProductPage" />
                    </div>
                </div>
            </div>
            ...
            <div class="col-12 col-lg-4">
                <div class="card mb-3">
                    <div class="card-header d-flex align-items-center">
                        ...
                        <button class="btn btn-sm btn-outline-secondary ms-auto" @click="loadTables"><i
                                class="bi bi-arrow-clockwise"></i></button>
                    </div>
                    <div class="card-body">
                        <div class="d-flex flex-wrap gap-2">
                            <button v-for="t in tables" :key="t.id" class="btn btn-sm" :class="getTableBtnClass(t)"
                                @click="selectTable(t)">
                                {{ t.name }}
                            </button>
                        </div>
                ...
                <div class="card">
                    <div class="card-header d-flex align-items-center">
                        ...
                        <div class="ms-auto small text-muted">{{ orderType === 'AT_TABLE' ? selectedTable?.name || 'Chưa
                            chọn bàn' : 'Mang về' }}</div>
                    </div>
                    <div class="card-body">
                        <div v-if="activeOrder">
                            <div class="list-group mb-3">
                                <div class="list-group-item d-flex align-items-center" v-for="it in activeOrder.items"
                                    :key="it.id">
                                    ...
                                    <div class="d-flex align-items-center gap-1">
                                        <button class="btn btn-sm btn-outline-secondary"
                                            @click="decreaseItem(it)">-</button>
                                        <span class="px-2">{{ it.quantity }}</span>
                                        <button class="btn btn-sm btn-outline-secondary"
                                            @click="increaseItem(it)">+</button>
                                        <button class="btn btn-sm btn-outline-danger ms-2" @click="removeItem(it)"><i
                                                class="bi bi-x"></i></button>
                                    </div>
                                </div>
                                <div v-if="!activeOrder.items || activeOrder.items.length === 0"
                                    class="text-center text-muted py-3">Chưa có món.</div>
                            </div>

                            <div class="d-flex align-items-center mb-2">
                                <input class="form-control" placeholder="Mã voucher" v-model="voucherCode" />
                                <button class="btn btn-outline-primary ms-2" @click="applyVoucherToOrder">Áp
                                    dụng</button>
                                <button class="btn btn-outline-secondary ms-2"
                                    @click="removeVoucherFromOrder">Gỡ</button>
                            </div>
                    ...
                    <div class="card-footer d-flex flex-wrap gap-2">
                        ...
                        <button class="btn btn-outline-danger" :disabled="!activeOrder" @click="cancelCurrentOrder">Hủy
                            đơn</button>
                        <div class="ms-auto d-flex gap-2">
                            <select class="form-select" style="max-width: 160px;" v-model="paymentMethod">
                                <option value="CASH">Tiền mặt</option>
                                <option value="TRANSFER">Chuyển khoản</option>
                                <option value="CARD">Thẻ</option>
                            </select>
                            <button class="btn btn-primary" @click="createOrSubmitOrder">{{ activeOrder ? 'Gửi bếp' :
                                'Tạo đơn' }}</button>
                            <button class="btn btn-success" :disabled="!activeOrder" @click="payCurrentOrder">Thanh
                                toán</button>
                        </div>
                    </div>
                </div>
...
const payCurrentOrder = async () => {
    if (!activeOrder.value) return
    try {
        const data = await payOrder(activeOrder.value.id, paymentMethod.value)
...
const cancelCurrentOrder = async () => {
    if (!activeOrder.value) return
    try {
        const data = await cancelOrder(activeOrder.value.id)
