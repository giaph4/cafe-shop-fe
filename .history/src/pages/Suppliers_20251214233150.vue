        ...
        <div class="suppliers-header__content">
          ...
          <div class="suppliers-header__actions">
            ...
            <button
              class="btn btn-primary"
              type="button"
              style="height: 40px; padding: 12px 20px; gap: 8px; font-family: var(--font-family-sans);"
              @click="openModal()"
            >
              <i
                class="bi bi-plus-lg"
        ...
        <div class="card-body">
          <div class="row g-3 align-items-end">
            ...
            <div class="col-lg-2 col-md-3">
              ...
              <select
                class="form-select"
                :value="tableData.pageSize.value"
                @change="tableData.setPageSize(Number($event.target.value))"
              >
                <option
      ...
      <div class="card table-card">
              ...
              <thead class="table-light">
                <tr>
                  ...
                  >
                    <input
                      type="checkbox"
                      :checked="bulkActions.isSelectAll && suppliers.length > 0"
        ...
        >
          ...
          <Pagination
            v-if="totalPages > 1"
            mode="zero-based"
            :current-page="tableData.zeroBasedPage.value"
            :total-pages="totalPages"
            @page-change="tableData.setPage"
          />
        </div>
      </div>
    </div>
...
const tableData = useTableData({
    fetchFn: async (params) => await getSuppliers({
        page: params.page,
        size: params.size,
...
const handleSelectAll = () => {
    if (bulkActions.isSelectAll.value) {
        ...
        bulkActions.selectAll(suppliers.value)
    }
}
...
const handleImportComplete = (result) => {
    ...
    // Invalidate queries sẽ được xử lý tự động bởi useTableData
}
...
const suppliers = computed(() => tableData.data.value || [])
...
const emailCount = computed(() => suppliers.value.filter((supplier) => supplier.email)?.length ?? 0)
const addressCount = computed(() => suppliers.value.filter((supplier) => supplier.address)?.length ?? 0)
...
const errorMessage = computed(() => {
    ...
    return err?.response?.data?.message || err?.message || 'Không thể tải dữ liệu nhà cung cấp.'
})
