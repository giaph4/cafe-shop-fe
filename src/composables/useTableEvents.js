...
const DESTINATION = '/topic/tables/status-updates'

/**
 * Composable để kết nối WebSocket và nhận real-time updates cho Table status
...
export const useTableEvents = (handleEvent) => {
    ...
    const clearSubscription = () => {
        if (subscription) {
            try {
                ...
                logger.warn('Failed to unsubscribe from table events', error)
            }
            subscription = null
        }
    }
    ...
    const cleanupClient = async () => {
        ...
        if (client.value) {
            try {
                ...
                logger.warn('Failed to deactivate table STOMP client', error)
            }
            client.value = null
        }
    ...
    const buildClient = () => {
        ...
        instance.onConnect = () => {
            ...
            // Subscribe to table status updates topic
            subscription = instance.subscribe(DESTINATION, (message) => {
                try {
                    ...
                    logger.warn('Failed to parse table event payload', error)
                }
            })
        }
        ...
        instance.onStompError = (frame) => {
            ...
            logger.warn('STOMP error received for table events', frame)
        }

        instance.onWebSocketError = (event) => {
            lastError.value = event
            logger.warn('WebSocket error for table events', event)
        }
