...
// Sorting (API hỗ trợ)
const sort = reactive({
    sortBy: 'expenseDate', // Mặc định backend
    sortDir: 'desc',
});

// Filtering (API hỗ trợ [cite: 417-424])
...
const form = reactive({
    ...
    description: '',
    amount: 0,
    expenseDate: new Date().toISOString().split('T')[0], // Mặc định hôm nay
});
...
// --- Handlers ---
const applyFilters = () => {
    pagination.pageNumber = 0; // Reset trang khi lọc
    fetchExpenses();
};
...
const handleSubmit = async () => {
    try {
        const dataToSend = {
            ...
            amount: form.amount,
            // Đảm bảo gửi đi đúng định dạng ISO
            expenseDate: new Date(form.expenseDate).toISOString(),
        };

        if (isEditMode.value) {
            // API: PUT /api/v1/expenses/{id} [cite: 449-451]
            await updateExpense(form.id, dataToSend);
        } else {
            // API: POST /api/v1/expenses [cite: 395-397]
...
const showModal = (expense = null) => {
    if (expense) {
        ...
        form.expenseDate = expense.expenseDate.split('T')[0]; // Định dạng YYYY-MM-DD
    } else {
