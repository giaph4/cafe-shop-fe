<template>
        ...
        <div class="modal-content">
          ...
          <Form @submit="handleSubmit" :validation-schema="tableSchema" v-slot="{ errors }">
            <div class="modal-body">
              <div class="mb-3">
                <label for="name" class="form-label fw-bold">Tên Bàn <span class="text-danger">*</span></label>
                <Field 
                  name="name" 
                  type="text" 
                  class="form-control"
              ...
              <div class="mb-3">
                <label for="capacity" class="form-label fw-bold">Số chỗ ngồi <span class="text-danger">*</span></label>
                <Field 
                  name="capacity" 
                  type="number" 
                  class="form-control"
              ...
              <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <div class="dropdown">
                  ...
                  <ul class="dropdown-menu dropdown-menu-end">
                    ...
                    <li>
                      <a class="dropdown-item text-danger" href="#" @click.prevent="handleDelete(table)">
  ...
  const formData = reactive({ id: null, name: '', capacity: 2 })
  const filterState = reactive({ name: '', status: '', capacity: '' })
  
  const tableSchema = yup.object({
    name: yup.string().required('Tên bàn là bắt buộc'),
    capacity: yup.number().required('Số chỗ là bắt buộc').min(1, 'Số chỗ phải lớn hơn 0').integer(),
  })
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err) => toast.error(err.response?.data?.message || 'Không thể xoá bàn.')
  })
  
  const statusMutation = useMutation({
    mutationFn: updateTableStatus,
    onSuccess: (data) => {
      toast.success(`Đã cập nhật bàn "${data.name}"`)
      queryClient.invalidateQueries(['tables'])
    },
    onError: (err) => {
  ...
  const handleSubmit = () => {
    const data = { name: formData.name, capacity: formData.capacity }
    if (isEditing.value) {
  ...
  // Cập nhật class và icon cho 5 trạng thái
  ...
  <style scoped>
  /* Thêm style cho 2 trạng thái mới */
  .status-available { border-color: #d1e7dd; background-color: #f6fbf9; }
  .status-available .table-icon { color: #198754; }
  .status-available .card-footer select { border-color: #198754; }
  
  .status-serving { border-color: #ffc107; background-color: #fffbf0; }
  .status-serving .table-icon { color: #A36B4A; }
  .status-serving .card-footer select { border-color: #A36B4A; }
  
  .status-reserved { border-color: #c14a45; background-color: #fcf2f2; }
  .status-reserved .table-icon { color: #c14a45; }
  .status-reserved .card-footer select { border-color: #c14a45; }
  
  .status-pending { border-color: #e2e3e5; background-color: #f8f9fa; }
  .status-pending .table-icon { color: #6c757d; }
  .status-pending .card-footer select { border-color: #6c757d; }
  
  .status-empty { border-color: #e2e3e5; background-color: #f8f9fa; }
  .status-empty .table-icon { color: #6c757d; }
  .status-empty .card-footer select { border-color: #6c757d; }
  
  .table-card { transition: all 0.3s ease; border-width: 2px; border-style: solid; }
  .table-card .card-header { background-color: transparent; border-bottom: 1px solid #eee; }
  .table-card .card-title { color: #3B2F2F; }
  .table-card .btn-icon { opacity: 0.6; }
  .table-card .btn-icon:hover { opacity: 1; color: #000; }
  .table-card .table-icon { font-size: 3.5rem; }
  .card-footer { background-color: #fdfaf7; }
  </style>